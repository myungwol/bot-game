...
class RankingView(ui.View):
    def __init__(self, user: discord.Member):
        ...
        self.category_map = {
            ...
            "voice": {"column": "voice_minutes", "name": "ボイスチャット", "unit": "分"},
            "chat": {"column": "chat_count", "name": "チャット", "unit": "回"},
            "fishing": {"column": "fishing_count", "name": "釣り", "unit": "匹"},
            "harvest": {"column": "harvest_count", "name": "収穫", "unit": "回"},
            "mining": {"column": "mining_count", "name": "採掘", "unit": "回"},
        }
    ...
    def build_components(self):
        ...
        category_select = ui.Select(
            placeholder="ランキングカテゴリを選択してください...",
    ...
    async def check_level_button(self, interaction: discord.Interaction, button: ui.Button):
        ...
        if retry_after:
            ...
            await interaction.response.send_message(
                f"⏳ しばらくしてからもう一度お試しください。(使用可能: {discord.utils.format_dt(available_at, style='R')})",
                ephemeral=True,
                delete_after=10
            )
            return
        ...
        try:
            ...
            error_message = "❌ ステータス情報の読み込み中にエラーが発生しました。"
...
class LevelSystem(commands.Cog):
    ...
    async def _build_champion_embed(self) -> discord.Embed:
        categories = {
            ...
            "harvest": {"column": "harvest_count", "name": "収穫", "unit": "回", "table": "total_stats"},
            "mining": {"column": "mining_count", "name": "採掘", "unit": "回", "table": "total_stats"},
        }
        ...
        if not server_id:
            ...
            return discord.Embed(title="エラー", description="SERVER_IDが設定されていません。")
        ...
        if not embed_template:
            return discord.Embed(title="エラー", description="チャンピオンボードのテンプレートが見つかりません。")
