# bot-game/cogs/system/panel_updater.py (강/바다 낚시터 인식 추가)
...
class PanelUpdater(commands.Cog):
    ...
    async def check_for_panel_updates(self):
        panel_map = {
            # [🔴 핵심 수정] 'fishing'을 'fishing_river'와 'fishing_sea'로 분리
            "fishing_river": {"cog_name": "Fishing", "channel_key": "river_fishing_panel_channel_id"},
            "fishing_sea":   {"cog_name": "Fishing", "channel_key": "sea_fishing_panel_channel_id"},
        ...
        for panel_key, info in panel_map.items():
            ...
            if float(request_timestamp) > last_checked:
                ...
                try:
                    # [🔴 핵심 수정] regenerate_panel에 panel_key를 전달하여 어떤 패널을 만들지 알려줌
