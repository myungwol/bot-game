...
class GuideSender(commands.Cog):
    ...
    async def send_guide(self, interaction: discord.Interaction, channel: discord.TextChannel):
        ...
        embed1 = discord.Embed(
            title="🏡 𝗗𝗜𝘀𝗰𝗼𝗿𝗱 𝗩𝗶𝗹𝗹𝗮𝗴𝗲へようこそ！ - ゲームボットガイド 🌟",
            description=(
        ...
        embed2.add_field(
            ...
            value=(
                ...
                "**- 2次（Lv.100）:** 1次職の上位職\n"
                "**- 方法:** レベル達成時に <#1423525617130803261> に個人スレッドが自動生成され、案内されます。"
            ),
            inline=False
        )
        ...
        embed3.add_field(name="**1. 釣り** <#1423528719573717065>, <#1423528777920680059>", value="釣り竿とエサを装着し、タイミングを合わせて魚を釣り上げましょう！大物や珍しい魚種を釣ることもできます。", inline=False)
        embed3.add_field(name="**2. 農業** <#1423529128195260467>", value="個人農場で畑を耕し、種を植え、水をやって作物を収穫しましょう。雨の日は水やりが自動で行われます！", inline=False)
        embed3.add_field(name="**3. 採掘** <#1423529224043499520>", value="`鉱山入場券`を使って個人鉱山に入場し、`ツルハシ`で鉱石を探して採掘しましょう。", inline=False)
        embed3.add_field(name="**4. 料理** <#1423528671536222330>", value="`釜`に様々な材料を組み合わせて料理を作りましょう。隠されたレシピを初めて発見して名声を得ることもできます。", inline=False)
        embed3.add_field(name="**5. 鍛冶屋** <#1423528443169214514>", value="鉱物とコインを使って各種道具をより高い等級にアップグレードできます。（24時間所要）", inline=False)
        ...
        embed4.add_field(name="**1. ペットを手に入れる** <#1423527808843644928>", value="所持している卵を孵化器に入れてペットを手に入れます。", inline=False)
        embed4.add_field(name="**2. ペットの管理と成長**（個人スレッド）", value="エサやり、遊び、ステータス分配、進化を通じてペットを成長させることができます。", inline=False)
        embed4.add_field(name="**3. ペットとの冒険**", value="**- 探検 <#1423527592144797726>:** ペットを探検に送り、報酬を得させます。\n**- 対戦 <#1422386181940838440>:** 他のユーザーのペットと腕を競います。", inline=False)
        embed4.add_field(name="**4. ボスレイド** <#1423527525992239184>, <#1423527555583180851>", value="すべての住民が力を合わせ、強力なボスを倒し、与えたダメージの順位に応じて報酬を受け取ります。", inline=False)
        ...
        embed5.add_field(name="**1. ミニゲーム（カジノ）**", value="<#1423528202927734794>、<#1423528236897407026>、<#1423528306971639889>のチャンネルで運試しをしてコインを獲得しましょう。", inline=False)
        ...
        embed6.add_field(name="**1. 商店** <#1423527977056080014>", value="アイテムを買ったり売ったりできます。相場は毎日少しずつ変動します。", inline=False)
        embed6.add_field(name="**2. 取引** <#1423528001970372688>, <#1423528036497756201>", value="他のユーザーにコインを送ったり、1対1でアイテム/コインを安全に交換したり、郵便を送ったりできます。", inline=False)
        embed6.add_field(name="**3. クエスト** <#1423525547702489180>", value="毎日/毎週与えられる簡単な目標を達成して報酬を受け取りましょう。すべて完了するとボーナスがあります！", inline=False)
        embed6.add_field(name="**4. 友達招待** <#1423523529667776652>", value="自分だけの無期限招待コードで友達を招待し、友達が正式な住民になると報酬を受け取れます。", inline=False)
        embeds_to_send.append(embed6)

        # 7. 締め
        embed7 = discord.Embed(
            description="何か質問があれば、いつでも管理者や他の住民に聞いてみてください！\n\n**それでは、楽しい村の生活をお送りください！**",
