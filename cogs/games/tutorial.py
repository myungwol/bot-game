...
class TutorialSystem(commands.Cog):
    ...
    async def check_step_condition(self, user: discord.Member, step: int) -> bool:
        ...
        try:
            if step == 1: # 출석체크
                ...
                return (act_fish.count or 0) > 0 and (act_sell.count or 0) > 0
            
            elif step == 9: # 괭이, 물뿌리개, 호박 씨앗 구매 (인벤토리 확인)
                inv = await get_inventory(user)
                has_hoe = any('괭이' in name for name in inv.keys()) # 장착 중일 수도 있으므로 인벤토리+장비 체크 필요하나, 보통 구매하면 인벤에 둠.
                gear = await get_user_gear(user)
                ...
                # 밭 상태 확인: 하나라도 심어져 있고(planted) 물을 준 상태(water_count > 0 or last_watered_at is today)
                # 편의상 planted 상태이기만 해도 인정해줌 (물주기는 튜토리얼에서 강제하기 힘듦)
                ...
                # user_activities에 mining 기록이 있는지 확인
                ...
                # upgrades 테이블에 기록이 있는지 확인 (진행 중인 것도 포함)
                res = await supabase.table('blacksmith_upgrades').select('count', count='exact').eq('user_id', str(uid)).execute()
                # 또는 이미 업그레이드 된 장비를 가지고 있는지 확인 (Tier > 1)
                # 여기서는 단순히 업그레이드 시도 기록만 봅니다.
                ...
                # 펫이 있거나(stage > 1) 혹은 알 상태(stage 1)라도 있으면 OK
                # get_user_pet은 stage > 1 만 가져오므로, 직접 쿼리
                ...
                # user_settings에 kitchen_thread_id가 있는지 확인
                res = await supabase.table('user_settings').select('kitchen_thread_id').eq('user_id', str(uid)).single().execute()
                return res.data and res.data.get('kitchen_thread_id') is not None
    ...
    async def regenerate_panel(self, channel: discord.TextChannel, panel_key: str = "panel_tutorial", **kwargs):
        ...
        # 새 패널 생성
        # 여기서는 유저마다 보여지는게 다르므로, "튜토리얼 확인하기" 버튼 하나만 있는 공용 패널을 만듭니다.
        embed = discord.Embed(
