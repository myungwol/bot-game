...
class ItemUsageView(ui.View):
    ...
    async def on_item_select(self, interaction: discord.Interaction):
        selected_item_key = interaction.data["values"][0]; usable_items_config = get_config("USABLE_ITEMS", {}); item_info = usable_items_config.get(selected_item_key)
        if not item_info:
            await interaction.response.defer(); self.parent_view.status_message = get_string("profile_view.item_usage_view.error_invalid_item"); return await self.on_back(interaction, reload_data=True)
        item_name_from_db = await self.get_item_name_by_id_key(selected_item_key)
        if not item_name_from_db:
            await interaction.response.defer(); self.parent_view.status_message = "❌ 아이템 정보를 DB에서 찾을 수 없습니다."; return await self.on_back(interaction, reload_data=True)
