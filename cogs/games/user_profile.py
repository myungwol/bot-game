...
class QuantityModal(ui.Modal):
    ...
    def __init__(self, title: str, label: str, placeholder: str, max_value: int):
        super().__init__(title=title); self.quantity.label = label; self.quantity.placeholder = placeholder
        self.max_value = max_value; self.value: Optional[int] = None
...
class ShopViewBase(ui.View):
    def __init__(self, user: discord.Member):
        super().__init__(timeout=300); self.user = user; self.message: Optional[discord.WebhookMessage] = None
        self.wallet_balance = 0; self.inventory: Dict[str, int] = {}; self.aquarium: List[Dict[str, Any]] = []
...
class SellItemView(ShopViewBase):
    def __init__(self, user: discord.Member):
        super().__init__(user); self.sell_categories = get_config("SHOP_SELL_CATEGORIES", ["魚", "アイテム"])
        self.current_category = self.sell_categories[0]
    async def _build_embed(self) -> discord.Embed:
        embed_data = await get_embed_from_db("embed_shop_sell")
        if not embed_data: return discord.Embed(title=f"📦 販売所 - 「{self.current_category}」", description=f"現在の所持金: `{self.wallet_balance:,}`{self.currency_icon}", color=discord.Color.orange())
    ...
    async def sell_callback(self, interaction: discord.Interaction):
        sell_type, target = interaction.data['values'][0].split('_', 1); uid_str = str(self.user.id); footer_msg = ""
        try:
            if sell_type == "fish":
                await interaction.response.defer()
                fish_id = int(target); fish_data = next((f for f in self.aquarium if f.get('id') == fish_id), None)
                ...
                self.wallet_balance += price; footer_msg = f"✅ {item_name} {qty}個を売却し、{price}{self.currency_icon}を獲得しました！"
...
class BuyItemView(ShopViewBase):
    def __init__(self, user: discord.Member):
        super().__init__(user); self.buy_categories = get_config("SHOP_BUY_CATEGORIES", ["里の役職", "釣り"])
        self.current_category_index = 0
    async def _build_embed(self) -> discord.Embed:
        category = self.buy_categories[self.current_category_index]; embed_data = await get_embed_from_db("embed_shop_buy")
        if not embed_data: return discord.Embed(title=f"🏪 Dico森商店 - 「{category}」", description=f"現在の所持金: `{self.wallet_balance:,}`{self.currency_icon}", color=discord.Color.blue())
    ...
    def _build_components(self):
        self.clear_items(); is_first, is_last = self.current_category_index == 0, self.current_category_index >= len(self.buy_categories) - 1
        prev_btn = ui.Button(label="◀ 前", disabled=is_first, row=0, custom_id="buy_cat_prev"); next_btn = ui.Button(label="次 ▶", disabled=is_last, row=0, custom_id="buy_cat_next")
        prev_btn.callback = self.nav_category_callback; next_btn.callback = self.nav_category_callback
    ...
    async def select_callback(self, interaction: discord.Interaction):
        name = interaction.data['values'][0]; data = get_item_database().get(name); user = interaction.user; uid_str = str(user.id); footer_msg = ""
        ...
        try:
            if data['category'] == '里の役職':
                ...
                if not role_id or not isinstance(user, discord.Member) or not (role := interaction.guild.get_role(role_id)): raise ValueError("Role not found.")
                ...
                await interaction.response.defer()
                rod_hierarchy = get_config("ROD_HIERARCHY", []); current_rank = -1
                for idx, rod in enumerate(rod_hierarchy):
                    if self.inventory.get(rod, 0) > 0 or rod == get_config("DEFAULT_ROD", "古い釣竿"): current_rank = idx
                if rod_hierarchy.index(name) <= current_rank: footer_msg = "❌ すでにその装備またはより良い装備を持っています。"; return await self.update_view(interaction, footer_msg)
                ...
                await interaction.response.send_modal(modal); await modal.wait()
                if modal.value is None: await self.update_view(interaction, "❌ 購入がキャンセルされました。"); return
            ...
            logger.error(f"구매 처리 중 오류: {e}", exc_info=True); footer_msg
